  ...
  return (
    <main className="min-h-screen bg-cream pt-16">
      <section className="px-6 py-20 text-center sm:py-24">
        <span className="inline-flex rounded-full bg-mint/15 px-4 py-2 text-sm font-bold text-mint-dark">KAKA INCLUDED WITH DESKBUB</span>
        <h1 className="mt-6 font-display text-5xl font-extrabold text-text-primary sm:text-6xl">Download DeskBub for Windows &amp; Mac</h1>
        <p className="mx-auto mt-5 max-w-2xl text-xl leading-relaxed text-text-secondary">Kaka comes with DeskBub at no cost. No account or payment required. If you&apos;ve created a custom pet, sign in to get your pairing code.</p>
      ...
      <section className="bg-white px-6 py-20"><div className="mx-auto max-w-5xl"><h2 className="text-center font-display text-3xl font-bold text-text-primary sm:text-4xl">Kaka appears in three steps</h2><div className="mt-10 grid gap-5 md:grid-cols-3">{[['1', 'Download DeskBub', 'Choose the installer for your computer.'], ['2', 'Install and open', 'Launch the app—no account, code, or payment needed.'], ['3', 'Meet Kaka', 'Choose five actions, set reminders, or hide Kaka in one click.']].map(([number, title, description]) => <article key={number} className="rounded-2xl border border-gray-100 bg-cream p-7"><span className="text-sm font-extrabold text-coral">STEP {number}</span><h3 className="mt-3 font-display text-lg font-bold text-text-primary">{title}</h3><p className="mt-2 text-sm leading-relaxed text-text-secondary">{description}</p></article>)}</div></div></section>

      <section className="px-6 py-20"><div className="mx-auto max-w-3xl"><h2 className="text-center font-display text-3xl font-bold text-text-primary">Download FAQ</h2><div className="mt-8 space-y-4">{[['Do I need an account?', 'Not for Kaka. You can download and use him without an account or payment. An account is needed only to create and pair a custom pet.'], ['Does DeskBub access my files or screen?', 'DeskBub displays the pet in its own transparent window. Sharing features prepare a file for you; you remain in control of anything uploaded to social media.'], ['Can I switch back to Kaka?', 'Yes. Use the desktop controls or tray menu to switch back to Kaka at any time.']].map(([q, a]) => <details key={q} className="rounded-2xl border border-gray-100 bg-white px-6 py-4"><summary className="cursor-pointer font-bold text-text-primary">{q}</summary><p className="mt-3 leading-relaxed text-text-secondary">{a}</p></details>)}</div>{!isSignedIn && <p className="mt-10 text-center text-text-secondary">Already created a custom pet? <Link href="/sign-in" className="font-bold text-coral underline underline-offset-4">Sign in for your pairing code.</Link></p>}</div></section>
      <Footer />
    </main>
  );
}
